...
async function main() {
  ...
  await page.waitForFunction(
    () => document.querySelector('#used-colors')?.textContent.includes('3 种颜色'),
    null,
    { timeout: 5000 },
  );
  ...
  assert.equal(
    await page.evaluate(() => document.querySelector('#canvas').height),
    1400,
    '画布高应为 (48+2)*28（无图例区）',
  );
  ...
  const layout = await page.evaluate(() => {
    ...
    const firstRow =
      [...document.querySelectorAll('header .tb-row')].find((row) =>
        row.querySelector('#btn-undo'),
      ) || document.querySelectorAll('header .tb-row')[1];
    const headerOrder = [
      'btn-undo',
      'btn-redo',
      'undo-info',
    ...
    return {
      outlineRemoved:
        !document.getElementById('chk-outline') && !document.getElementById('dlg-outline'),
      recordsBeforeAutosave:
        headerOrder[0] >= 0 &&
        headerOrder[2] >= 0 &&
        headerOrder[0] < headerOrder[2] &&
        headerOrder[2] < headerOrder[3],
      autosaveBeforeExport:
        headerOrder[3] >= 0 && headerOrder[4] >= 0 && headerOrder[3] < headerOrder[4],
      codesAfterZoom: canvasOrder[0] >= 0 && canvasOrder[1] >= 0 && canvasOrder[0] < canvasOrder[1],
      emptyAfterCodes:
        canvasOrder[1] >= 0 && canvasOrder[2] >= 0 && canvasOrder[1] < canvasOrder[2],
      noRecordsTitle: ![...document.querySelectorAll('#right-panel .panel-title')].some(
  ...
  assert.ok(
    layout.recordsBeforeAutosave && layout.autosaveBeforeExport,
    '记录应在自动保存左侧、自动保存应在导出左侧',
  );
  assert.ok(
    layout.codesAfterZoom && layout.emptyAfterCodes,
    '显示色号/透明色应在画布工具栏缩放按钮右侧且顺序正确',
  );
  assert.ok(
    layout.noRecordsTitle &&
  ...
  await page.waitForFunction(
    () => {
      ...
      return (
        queue.length === 0 && !document.querySelector('#toast').textContent.includes('重新压缩')
      );
    },
    null,
    { timeout: 8000 },
  ...
  assert.ok(
    !(await page.textContent('#toast')).includes('重新压缩'),
    '首次打开网站不应弹出色板配置提示',
  );
  ...
  // 1.5 侧边栏折叠 / 展开：收起后宽度收缩、工作区变宽，展开后恢复
  {
    ...
    for (const { id, trigger } of panelSpec) {
      ...
      assert.ok(
        await page.evaluate(
          (pid) => document.getElementById(pid).classList.contains('collapsed'),
          id,
        ),
        `${id} 点击折叠按钮后应收起`,
      );
      const w = await page.evaluate(
        (pid) => Math.round(document.getElementById(pid).getBoundingClientRect().width),
        id,
      );
      ...
      if (id === 'left-panel') {
        ...
        assert.ok(
          Math.abs(panX - (panBefore + 288)) < 2,
          `折叠左侧栏后画布应补偿位移保持绝对位置：${panBefore} -> ${panX}`,
        );
      }
    }
    ...
    assert.ok(
      canvasAfter > canvasBefore,
      `折叠全部面板后工作区应变宽：${canvasBefore} -> ${canvasAfter}`,
    );
    ...
    assert.ok(
      Math.abs(panAfter - panBefore) < 2,
      `全部展开后画布位置应复原：${panBefore} -> ${panAfter}`,
    );
  ...
  // 1.7 颜色距离延迟生效：修改后需「重新压缩」才重新生成图案
  {
    ...
    await page.waitForTimeout(300);
    assert.equal(
      await page.evaluate(() => window.__app.settings.useLab),
      false,
      '修改颜色距离后应保存设置',
    );
    ...
    await page.waitForFunction(
      () => document.querySelector('#toast').textContent.includes('重新压缩'),
      null,
      { timeout: 3000 },
    );
  ...
  // 1.6 对比原图 / 同步拖拽
  {
    ...
    assert.ok(
      await page.evaluate(() =>
        document.querySelector('#canvas-scroll').classList.contains('compare-on'),
      ),
      '勾选对比后工作区应分为左右两块',
    );
    assert.ok(
      await page.evaluate(() => document.querySelector('#canvas-original').width > 0),
      '左侧原图画布应有内容',
    );
    ...
    assert.ok(
      await page.evaluate(() => document.querySelector('#chk-sync-pan').checked),
      '对比开启时可直接勾选同步',
    );
    ...
    await page.waitForTimeout(350);
    assert.ok(
      await page.evaluate(() => document.querySelector('#chk-compare').checked),
      '未勾选对比时勾选同步应自动勾选对比',
    );
    ...
    const initConsistent = await page.evaluate(() => {
      ...
      return (
        Math.abs(a.origPan.x - (a.pan.x + 1 * 28 * a.zoom)) < 1 &&
    ...
    assert.ok(
      Math.abs(afterPan.ox - expectOx) < 1,
      `同步拖拽下原图 pan 应含边距换算：${afterPan.ox.toFixed(1)} vs ${expectOx.toFixed(1)}`,
    );
    assert.ok(
      Math.abs(afterPan.oz - afterPan.z * CELL) < 1e-6,
  ...
  assert.equal(
    await page.evaluate(
      () => getComputedStyle(document.querySelector('#selection-controls')).display,
    ),
    'none',
    '画笔模式应隐藏同色选区与选中高亮',
  );
  ...
  assert.ok(
    (await page.textContent('#used-colors')).includes('4 种颜色'),
    '编辑后当前使用颜色数应同步更新',
  );
  const edge = await px(
    page,
    OP + MARGIN + 10 * CELL + 3,
    OP + MARGIN + 10 * CELL + Math.floor(CELL / 2),
  );
  ...
  await page.keyboard.press('Control+z');
  await page.waitForTimeout(250);
  await near(await px(page, ccx(10), ccy(10)), [229, 57, 53], 10, '再次撤销后应恢复原红色');
  await page.keyboard.press('Control+Shift+z');
  await page.waitForTimeout(250);
  await near(
    await px(page, ccx(10), ccy(10)),
    [255, 255, 255],
    10,
    'Ctrl+Shift+Z 重做后应恢复白色',
  );
  assert.equal(
  ...
  assert.ok(
    Math.abs(hatch[0] - 236) < 18 && Math.abs(hatch[1] - 236) < 18 && Math.abs(hatch[2] - 236) < 18,
    `空位应为浅灰斜线底色，实际 ${hatch}`,
  );
  ...
  assert.equal(
    await page.locator('#quick-picker button:not(.qp-cancel)').count(),
    9,
    '应弹出 9 个备选色',
  );
  const qpBgs = await page.evaluate(() =>
    Array.from(document.querySelectorAll('#quick-picker button:not(.qp-cancel)')).map(
      (b) => b.style.background,
    ),
  );
  assert.ok(
    !qpBgs.some((bg) => hexToRgb(bg).every((v, i) => v === [255, 255, 255][i])),
    '九宫格应排除自身颜色（白色）',
  );
  const firstBtnText = await page
  ...
  assert.ok(
    await page.locator('#quick-picker').evaluate((el) => el.classList.contains('hidden')),
    '九宫格打开后直接拖拽应自动关闭',
  );
  ...
  assert.ok(
    wandModeBox && wandTolBox && Math.abs(wandModeBox.y - wandTolBox.y) < 2,
  ...
  assert.ok(
    selPx[0] < 120 || (selPx[0] > 200 && selPx[1] > 200 && selPx[2] > 200),
  ...
  assert.ok(
    Math.abs(afterPan - beforePan) > 20,
    `工作区拖拽应平移图片：${beforePan} -> ${afterPan}`,
  );
  ...
  const hcCounts = await page.evaluate(() =>
    Array.from(document.querySelectorAll('#highlight-color-list .hc-count')).map((e) =>
      parseInt(e.textContent.replace('×', ''), 10),
    ),
  );
  assert.ok(
    hcCounts.every((v, i) => i === 0 || hcCounts[i - 1] <= v),
    '颜色清单应按数量正序排列',
  );
  ...
  await page.waitForTimeout(120);
  const baseRed = await px(
    page,
    OP + MARGIN + 5 * CELL + 2,
    OP + MARGIN + 5 * CELL + Math.floor(CELL / 2),
  );
  assert.ok(
    baseRed[0] > 150 && baseRed[1] < 130 && baseRed[2] < 130,
    `前置：样本点应为红色，实际 ${baseRed}`,
  );
  ...
  const redIdx = await page.evaluate(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (m && Math.abs(+m[0] - 229) < 8 && Math.abs(+m[1] - 57) < 8 && Math.abs(+m[2] - 53) < 8)
        return i;
    }
    return -1;
  });
  ...
  await page.waitForTimeout(150);
  const frame2 = await px(
    page,
    OP + MARGIN + 5 * CELL + 2,
    OP + MARGIN + 5 * CELL + Math.floor(CELL / 2),
  );
  assert.ok(
    Math.abs(frame2[0] - baseRed[0]) <= 12 &&
      Math.abs(frame2[1] - baseRed[1]) <= 12 &&
      Math.abs(frame2[2] - baseRed[2]) <= 12,
  ...
  const blueIdx = await page.evaluate(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      const sw = items[i].querySelector('.swatch');
      const m = String(sw && getComputedStyle(sw).backgroundColor).match(/\d+/g);
      if (m && Math.abs(+m[0] - 30) < 8 && Math.abs(+m[1] - 136) < 8 && Math.abs(+m[2] - 229) < 8)
        return i;
    }
    return -1;
  });
  ...
  await page.waitForTimeout(150);
  const frameReplace = await px(
    page,
    OP + MARGIN + 5 * CELL + 2,
    OP + MARGIN + 5 * CELL + Math.floor(CELL / 2),
  );
  assert.ok(
    Math.abs(frameReplace[0] - baseRed[0]) <= 12 &&
      Math.abs(frameReplace[1] - baseRed[1]) <= 12 &&
      Math.abs(frameReplace[2] - baseRed[2]) <= 12,
    `选择另一种色号应清除前一种高亮，实际 ${frameReplace} vs ${baseRed}`,
  );
  assert.ok(
    await page
      .locator('#highlight-color-list .hc-item')
      .nth(blueIdx)
  ...
  await page.waitForFunction(
    () => document.querySelector('#export-dialog').classList.contains('hidden'),
    null,
    { timeout: 5000 },
  );
  ...
  assert.notEqual(
    fs.statSync(outJpgNoLegend).size,
  ...
  assert.notEqual(
    fs.statSync(outJpgNumbers).size,
    fs.statSync(outJpg).size,
    '行列号开关应改变导出图片',
  );
  await page.waitForFunction(
  ...
  await page.waitForFunction(
    async () => {
      try {
        ...
        if (!s.history || !Array.isArray(s.history.items) || s.history.items.length !== 1)
          return false;
        const g = s.project?.grid;
  ...
  console.log(
    '落盘状态:',
    stateNow.project ? '有 project' : '无 project',
    'project.grid:',
    stateNow.project && Array.isArray(stateNow.project.grid)
      ? `数组长度 ${stateNow.project.grid.length}`
      : stateNow.project
  ...
  await page.waitForFunction(
    () => document.querySelector('#config-select')?.options.length > 0,
    null,
    { timeout: 8000 },
  );
  await page.waitForFunction(() => document.querySelector('#canvas')?.width > 0, null, {
  ...
  assert.ok(
    restoredGrid && restoredGrid.cell1010 === -1 && restoredGrid.cell1110 === -1,
    `刷新后网格应保留擦除与涂色状态，实际 ${JSON.stringify(restoredGrid)}`,
  );
  assert.ok(
    Math.abs(restoredHatch[0] - 236) < 18 &&
      Math.abs(restoredHatch[1] - 236) < 18 &&
      Math.abs(restoredHatch[2] - 236) < 18,
  ...
  const detailResp = page.waitForResponse(
    (r) =>
      r.url().includes(`/api/configs/${encodeURIComponent(paletteCfgName)}`) &&
  ...
  assert.equal(
    await page.evaluate(() => window.__app.palette[0].hex),
    '#00FF00',
    '色板配置本身应已更新',
  );
  await page.waitForFunction(
...
`;
    ...
    assert.ok(
      Math.abs(kCheck.origZoom - kCheck.zoom * 28 * (kCheck.gridW / kCheck.dispW)) < 1e-6,
      `原图 zoom 应 = 拼豆 zoom × 28 × (网格宽/原图显示宽)，实际 ${kCheck.origZoom} vs ${kCheck.zoom * 28 * (kCheck.gridW / kCheck.dispW)}`,
    );
    assert.ok(
      Math.abs(kCheck.origPanX - (kCheck.panX + 1 * 28 * kCheck.zoom)) < 1,
      '原图 pan 应含 1 格行列号条偏移',
    );
    await page.uncheck('#chk-sync-pan');
    await page.uncheck('#chk-compare');
