...
function backendRender(gridLines) {
  ...
  fs.writeFileSync(
    spec,
    JSON.stringify({ w: W, h: H, grid: GRID, palette: PALETTE, cell: 8, gridLines }),
  );
...
function compare(name, front, back, tol, maxBadRatio) {
  assert.equal(
    front.length,
    back.length,
    `${name}: 两端输出字节数不一致（${front.length} vs ${back.length}）`,
  );
  let bad = 0;
  let maxDiff = 0;
  ...
  console.log(
    `[数据] ${name}: maxDiff=${maxDiff}，超差(${tol})像素比例=${(ratio * 100).toFixed(3)}%`,
  );
  assert.ok(
    ratio <= maxBadRatio,
    `${name}: 差异像素比例 ${(ratio * 100).toFixed(3)}% 超过 ${(maxBadRatio * 100).toFixed(2)}%`,
  );
}
...
async function main() {
  ...
  log('[步骤] 页面加载完成');
  await page.waitForFunction(() => !!window.__testHooks?.drawPattern, null, {
    timeout: 5000,
  });
  log('[步骤] 测试钩子就绪');
