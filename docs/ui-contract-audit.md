# UI 控件契约审计表
> 维护说明：本表是**设计说明**，不是唯一事实来源。可执行契约以代码与测试为准——
> tests/dom_contract_test.mjs（状态边界 / 设计令牌 / 弹窗行为）、tests/els_contract_test.mjs（DOM 注册表）、
> tests/dom_ui_test.mjs / tests/ui_test.mjs（交互行为）等。本表与代码冲突时，以测试与代码为准，并请同步更新本表。
